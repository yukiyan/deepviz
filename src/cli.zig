...
test "defaults" {
    const argv = [_][:0]const u8{};
    const args = try parse(&argv);
    try std.testing.expectEqualStrings("gemini-2.0-flash-preview-image-generation", args.model);
    try std.testing.expectEqualStrings("16:9", args.aspect_ratio);
    try std.testing.expectEqualStrings("2K", args.image_size);
    try std.testing.expect(!args.no_open);
}
