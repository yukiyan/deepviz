...
pub fn load(allocator: std.mem.Allocator, args: cli.Args) !Config {
    ...
    // CLI overrides
    if (args.model.len > 0 and !std.mem.eql(u8, args.model, "gemini-2.0-flash-preview-image-generation")) cfg.model = args.model;
    if (!std.mem.eql(u8, args.aspect_ratio, "16:9")) cfg.aspect_ratio = args.aspect_ratio;
    if (!std.mem.eql(u8, args.image_size, "2K")) cfg.image_size = args.image_size;
